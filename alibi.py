...
class ALiBi(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        slopes = torch.pow(2.0, -torch.arange(1, self.n_heads + 1, dtype=torch.float32) * (8.0 / self.n_heads))
    ...
    def get_bias(self, seq_len: int, device: torch.device) -> Float[Tensor, "n_heads seq_len seq_len"]:
        ...
        distance = (pos_i - pos_j).abs()  # [seq_len, seq_len] - distance between positions
        
        # Apply slopes: -slope[h] * distance
        # slopes: [n_heads] -> [n_heads, 1, 1]
        # distance: [seq_len, seq_len] -> [1, seq_len, seq_len]
        slopes_expanded = self.slopes.unsqueeze(-1).unsqueeze(-1)  # [n_heads, 1, 1]
